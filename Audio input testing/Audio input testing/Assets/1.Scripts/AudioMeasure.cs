...
public class AudioMeasure : MonoBehaviour
{
    ...
    public AudioClip testingAudio;

    //microphone
    private MicrophoneScript micScript;
    ...
    void Start()
    {
        ...
        micScript = GetComponent<MicrophoneScript>();
    }

    void Update()
    {
        if (testingStart && !audioSource.isPlaying) 
        {
            audioSource.clip = testingAudio;
            audioSource.Play(); 
        }
        else if (!micScript.microphoneListenerOn && !testingStart)
        { audioSource.Stop(); }
        if (micScript.microphoneListenerOn || testingStart)
        {
            AnalyzeSound();
